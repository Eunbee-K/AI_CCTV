...
> **2026-08-15 갱신 — 모델 개발은 마감됐다.** 아래 v6 계획은 실행되지 않았고,
> 최종 구조는 **분류기(다중클래스) + YOLO test3 병렬**로 확정됐다
> ([필터C+test3 병렬 확정](2026-08-15-필터C-test3-병렬-확정.md)).
> 이 스윕 시스템으로 새 YOLO를 학습할 계획은 현재 없다.

~~**v6(2026-08-10 예정)** — v5의 `per_class_metrics.json`에서 정확도가 높은 항목만 추려
설정 파일의 `classes` 블록을 새로 구성하면 된다.~~ (미실행, 위 갱신 참고)

시스템 자체는 손댈 필요 없이 **설정 파일만 바꾸면 되는 구조**다 — 나중에 다시
학습할 일이 생기면 그대로 쓸 수 있다.

---

## 8. `used_files.csv`는 유출 검증에 쓰라 (2026-08-15 추가)

§5의 "디스크 회수와 추적 가능성"에서 만든 `used_files.csv`가 실제로 결정적인
역할을 했으므로 용도를 명시해둔다.

**검증셋을 새로 만들 때는 반드시 이 파일과 먼저 대조할 것.** 2026-08-15에
`testset_bycode`(결함 455장)를 test5의 `used_files.csv`와 대조해보니
**259장(56.9%)이 test5의 train에 그대로 들어 있었다.** 그 판으로 잰 YOLO 이름
정확도 90.0%는 일반화가 아니라 암기를 잰 값이었고, 유출 없는 판에서 다시 재니
33.3%였다.

```python
# 대조 방법 — source_path(원본 절대경로)의 파일명으로 비교한다
used = {Path(r["source_path"]).name
        for r in csv.DictReader(open(run_dir / "used_files.csv", encoding="utf-8-sig"))}
overlap = used & {Path(r["src"]).name for r in validation_manifest}
```

`dest_filename`은 `yolo_txt_BK__원본명.png`처럼 접두어가 붙으므로 그대로 비교하면
안 된다(0개로 나와 유출이 없는 것처럼 보인다 — 실제로 한 번 그렇게 헛짚었다).

`deterministic_pick`이 시드 고정이라 yaml만 있으면 재현도 가능하지만, `used_files.csv`가
있으면 **그 run이 실제로 쓴 파일**을 직접 알 수 있으니 이쪽이 확실하다.
...
[`docs/reports/2026-08-15-필터C-test3-병렬-확정.md`](2026-08-15-필터C-test3-병렬-확정.md) (최종 확정 구조·유출 검증)
