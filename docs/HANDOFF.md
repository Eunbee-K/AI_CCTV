...
1. **필터가 실영상에서 무작위다.** 야장 val AUC 0.9503인데 SM2 실영상 0.5205.
   학습 데이터에 "조사원이 기록한 순간"만 있고 그 사이 평범한 구간이 없는 게 원인.
   → 연속 영상의 평범한 구간을 정상으로 넣어 다시 학습. 이게 지금 최대 병목이다.
   지금 `lead` 모드가 도는 건 "필터가 고른 20%"에 결함이 우연히 섞여 있어서다.
