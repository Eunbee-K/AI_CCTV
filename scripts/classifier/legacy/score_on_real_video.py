...
*** 경고 (2026-08-15): 이 스크립트가 쓰는 정답은 폐기됐다. ***

`filter_eval_SM2*.csv`의 `is_defect`는 사람이 프레임 단위로 찍은 것이 아니라
**조사표 거리 ±1m를 기계적으로 펼쳐** 만든 것이다. 카메라가 느려 그 ±1m가 프레임
수십 장에 걸치는 바람에 정답 "결함" 구간이 전체의 24.6%(한 관로는 100%)까지
부풀었다. 이 정답으로 잰 **AUC 0.5205는 필터가 아니라 라벨을 잰 값**이라 폐기했다.

**이 스크립트를 다시 쓰려면 정답 CSV부터 다시 만들어야 한다** — 연속 영상에 사람이
프레임 단위로 결함 유무를 찍은 것으로. 그 전에는 여기서 나오는 숫자를 인용하지 말 것.
