...
### 2026-08-12 세션 — 필터 실영상 검증 실패 → lead 구조 + LLM 병렬
...
- **필터가 실영상에서 무작위임이 드러났다(줄기 3).** 야장 val AUC 0.9503이 배포본
  ONNX에서도 그대로 재현되는데, 실제 연속 영상에서는 **AUC 0.5205**다. 결함 중앙값
  0.9420 / 그 외 0.9380으로 갈라지지 않는다. v1(AIHub)이 현장에서 0.516이었을 때와
  같은 모양. 8/11에 "상위 20%로 잘라도 유실 0건"이라 본 것은 착시였다 — 정답 구간이
  전체의 24.6%라 **무작위로 골라도 유실이 0.4건**이다. 원인은 학습 데이터에 조사원이
  기록한 순간만 있고 그 사이 평범한 구간이 없다는 것.
...
- **흑백 학습은 기각(줄기 3).** 학습 데이터가 실영상보다 확연히 푸르고 진해서
  (R-B −16~−27 vs −4~−8) 색이 지름길일 수 있다고 봤는데, 지웠더니 야장 val은
  그대로고(0.9503→0.9517) 실영상도 그대로다(0.5205→0.5259).
...
- **현장 간 검증을 한 번도 안 해봤다.** val이 train과 같은 3개 현장이다
  (관로만 나눔). 야장 val 0.95와 실영상 0.52의 간극이 여기서 나온다.
...
- **(줄기 2) v6 최종 모델 구성** — 08-10 예정이었으나 지연 중. v5(18종 mAP50 0.703)에서
  ...
  (2종 0.821 → 5종 0.759 → 18종 0.703).
- **(줄기 3) 필터 재학습 — 지금 최대 병목.** 실영상 AUC 0.52다(8/12 검증 완료).
  학습 데이터에 연속 영상의 평범한 구간을 정상으로 넣어야 한다. 지금 `lead` 모드가
  그럭저럭 도는 건 상위 20%에 결함이 우연히 섞여서다.
- **(줄기 2·3) 변형(DF)을 아무도 못 잡는다.** test5에 DEFORM 클래스가 있는데 실영상에서
  한 번도 발화하지 않았고(conf 0.02까지 내려도) 필터도 이 유형이 제일 약하다.
