...
- ~~**필터가 실영상에서 무작위다 — AUC 0.5205.**~~ **폐기(2026-08-15).** 이 측정에
  쓴 검증셋이 잘못 만들어졌다 — 정답을 프레임 단위로 찍지 않고 **조사표(야장) 거리
  기록에서 기계적으로 펼쳤다.** 그 결과 정답 "결함" 구간이 전체의 24.6%(한 관로는
  100%)까지 부풀어, 실제로는 평범한 관 내부인 프레임이 대량으로 결함 정답이 됐다.
  **0.52는 필터가 아니라 그 라벨을 잰 값이다.** 이 숫자로 필터 성능을 말하지 말 것.
  필터의 현재 근거는 야장 val(AUC 0.9503)과
  [8/15 확정 보고서](docs/reports/2026-08-15-필터C-test3-병렬-확정.md)뿐이고,
  **실영상 성능은 아직 측정된 적이 없다** — 프레임 단위 정답을 만들어 다시 재야 한다.
...
- **흑백 학습은 기각(줄기 3).** 학습 데이터가 실영상보다 확연히 푸르고 진해서
  ...
  그대로였다(0.9503→0.9517). ~~실영상도 그대로(0.5205→0.5259)~~ — 실영상 쪽
  비교는 폐기된 검증셋이라 근거가 못 된다(위 8/12 항목 참고). 기각 판단 자체는
  야장 val만으로도 유지된다.
...
- **현장 간 검증을 한 번도 안 해봤다.** val이 train과 같은 3개 현장이다
  (관로만 나눔). 이건 여전히 사실이고 실제 위험이다 — 다만 그 근거로 들었던
  "실영상 0.52"는 폐기됐다(검증셋 오류).
...
- **(줄기 3) 필터 실영상 검증 — 지금 최우선.** 재학습이 아니라 **제대로 재는 것**이
  먼저다. 8/12의 AUC 0.52는 검증셋 오류로 폐기됐고(조사표 거리를 프레임 정답으로
  펼침 → 정답 구간 24.6%), 그래서 **실영상 성능을 아는 사람이 아무도 없다.**
  연속 영상 몇 개에 사람이 프레임 단위로 정답을 찍어 다시 재고, 그 결과를 보고
  재학습 여부를 정한다.
