    ...
    def add_video(self, path: Path):
        ...
        if path.name not in self.video_data_map:
            self.video_data_map[path.name] = {
                "path": path,
                # **등록 시점에는 비워둔다.** 관로번호는 영상 자막에서 읽는
                # 값이고, 그건 분석을 돌려야 알 수 있다. 예전에는 파일명을
                # 임시로 넣었는데, 업로드하자마자 화면에 파일명이 관로번호로
                # 박혀 보여서 "파일명이 그대로 들어간다"는 오해를 낳았다.
                # (파일명은 현장 일련번호인 경우가 많아 실제 관로번호와 다르다 —
                #  `5982.mp4`의 자막 관로번호는 `SM1-121-002`.)
                # 분석하면 analysis.py가 OCR 값으로 채우고, 자막을 못 읽었을
                # 때만 파일명으로 물러선다.
                "pipe_id": "",
                "dia": "",
                "rows": [],
