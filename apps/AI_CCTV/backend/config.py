...
#
# **필터의 실영상 성능은 아직 측정된 적이 없다.**
#
# 배포 모델은 야장 val에서 AUC 0.9503이고 그 값은 배포본(ONNX)에서도 정확히 재현된다.
#
# 한때 "실영상 AUC 0.5205 — 무작위"라고 적혀 있었으나 **2026-08-15에 폐기했다.**
# 그 측정의 정답을 사람이 프레임 단위로 찍지 않고 **조사표 거리 ±1m를 기계적으로
# 펼쳐** 만들었는데, 카메라가 느려 그 ±1m가 프레임 수십 장에 걸쳤다. 결과적으로
# 정답 "결함" 구간이 전체 프레임의 24.6%(한 관로는 100%)까지 부풀었고, 실제로는
# 평범한 관 내부인 프레임이 대량으로 결함 정답이 됐다.
# **0.52는 필터가 아니라 그 라벨을 잰 값이다. 인용하지 말 것.**
#
# 제대로 재려면 연속 영상에 사람이 프레임 단위로 정답을 찍어야 한다. 그 전까지
# 필터에 대해 말할 수 있는 것은 야장 기준 수치뿐이다
# (docs/reports/2026-08-15-필터C-test3-병렬-확정.md).
#
# **이 모델은 노후관로(우수관) 자료가 중심이다.** 신설 전용 모델(NEW_v1)이 AUC 0.75로
# 미달이라, 지금은 관로구분과 무관하게 돌린다(analysis.py `_run_filter` 참고).
# 2026-08-12 오후: lead. 필터의 실영상 순위 품질은 아직 검증되지 않았지만,
...
# 분류기 점수가 이 값 미만인 프레임의 **YOLO 검출은 버린다.**
# YOLO는 관 밖(맨홀·지상 전경)을 결함이라 부른다 — 테스트셋 실측 OUT_MH 89% /
# OUT_CAR 82% / OUT_INVERT 82%. 분류기는 같은 사진을 0%로 정확히 본다.
# 테스트셋 455결함 + 관밖 40장 실측 — 관 밖을 얼마나 버리고 결함을 얼마나 잃나:
#   0.05  관밖 72% 버림 · 결함 0장 손해
#   0.10  관밖 95% 버림 · 결함 1장(0.2%) 손해   ← 기본
#   0.20  관밖 98% 버림 · 결함 6장(1.3%) 손해
YOLO_DROP_BELOW = float(os.getenv("YOLO_DROP_BELOW", "0.10"))
...
# 조사 시작지점을 '조사시작' 자막으로 찾던 OCR_FIND_START는 2026-08-15에 제거했다.
# 관 밖 구간은 자막이 아니라 거리 판독(OCR)과 분류기의 관 외부 인식으로 걸러낸다.
# 영상은 항상 0초부터 끝까지 본다.
