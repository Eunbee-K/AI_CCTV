...
#
#   off       쓰지 않는다
#   series    점수 상위 FILTER_TOP_RATIO만 YOLO에 넘긴다(기본). 나머지는 버린다.
#   parallel  모든 프레임을 양쪽에 넣고 두 판단을 대조한다. 아무것도 버리지 않는
#             대신 YOLO 오탐/누락 의심을 표에 표시한다.
#
# 2026-08-12 기준 series. 배포 모델은 OLD_v3(야장 12,051장 + AIHub 정상 + S20 결함 보강).
#
...
#
# **series(직렬)** — 상위 20%만 YOLO에 넘긴다. SM2 8관로 실측에서 조사표 결함
# 15건을 하나도 잃지 않고 프레임 80.3%를 걸렀다. YOLO에 넣는 장수가 1/5이 된다.
#
# 앞서 "직렬은 결함 8.3%를 잃고 시간은 1%만 아낀다"고 봤던 것은 절대 임계값을 쓰던
# 때의 수치다. 순위 기반으로 바꾸니 결과가 뒤집혔다.
#
# **이 모델은 노후관로(우수관) 전용이다.** 관로구분이 '신설'이면 자동으로 건너뛴다
# (analysis.py). 신설용 필터는 데이터 부족으로 보류 — 현장 1,404장으로는 AUC 0.75.
FILTER_MODE = os.getenv("FILTER_MODE", "series").strip().lower()
FILTER_MODEL_PATH = Path(
    os.getenv("FILTER_MODEL_PATH", str(resource_path("assets/filter.onnx")))
)
# **절대 임계값은 쓰지 않는다.** 임계값은 점수 분포에 붙어 있는 값인데, 학습·검증에
# 쓴 야장 데이터는 결함 비율이 45%고 실제 영상은 1.5%다. 분포가 다르면 임계값이
# 옮겨가지 않는다. 야장 val에서 "결함 99% 재현"이던 0.032를 실영상에 그대로 썼더니
# 프레임의 0.6%밖에 안 걸러졌다.
#
# 대신 **영상마다 점수 상위 FILTER_TOP_RATIO만 남긴다.** 순위는 분포가 달라도
# 유지되므로 영상 조건이 달라도 자동으로 맞춰진다.
#
# SM2 8개 관로 1,014프레임 · 조사표 결함 15건 실측:
#   상위 10% -> 결함 2건 유실 · 프레임 90.3% 제거
#   상위 15% -> 결함 1건 유실 · 프레임 85.4% 제거
#   상위 20% -> 결함 0건 유실 · 프레임 80.3% 제거   <- 채택
#   상위 25% -> 결함 0건 유실 · 프레임 75.2% 제거
#
# 표본이 8관로 15건이고 결함 유형이 이음부 벌어짐에 몰려 있다. 학습이 부족한
# 유형(변형 등)은 순위가 낮게 나오므로 여유를 둬 20%로 잡았다.
